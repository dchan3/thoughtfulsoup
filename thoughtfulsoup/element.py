...
from thoughtfulsoup.multichecker import MultiChecker
    ...
    def select(self, selector, _candidate_generator=None, limit=None):
        ...
        for index, token in enumerate(tokens):
            ...
            if m is not None:
                ...
                if booty is not None:
                    ...
                    if pseudo_type in SUPPORTED:
                        ...
                        checker = pseudo_node["f"]
                    elif pseudo_type in ["not", "is"]:
                        selectors = []
                        checker_list = []
                        if ',' in pseudo_type:
                            selectors = re.split(',', pseudo_value)
                        else:
                            selectors.append(pseudo_value)
                        for s in selectors:
                            if re.search('^\w+', s) is not None:
                                tag_nombre = re.search('^\w+', s).group()
                                checker_list.append(lambda t, ts, tsf: t.name == tag_nombre)

                            if '#' in s:
                                id_name = re.search('#[\w\d]+', s)
                                if id_name is not None:
                                    checker_list.append(lambda t, ts, tsf: ID_CLASS_CHECKER["#"](t, id_name.group()))

                            if '.' in s:
                                class_find = re.search('(\.[\w\d]+)+', s)
                                if class_find is not None:
                                    checker_list.append(lambda t, ts, tsf: ID_CLASS_CHECKER["."](t, class_find.group()))

                            if ':' in s:
                                pseu_sel = s.split(":", 1)[1]
                                pseu_val = 1
                                if "(" in pseu_sel:
                                    pseu_val = re.search('\((.*)\)', pseu_sel).group(1)
                                    pseu_sel = pseu_sel.replace("(" + pseu_val + ")")
                                    if "n" in pseu_val and re.match(r'(\d+n[+-]\d+)|(\d+[+-]\d+n)|(-?\d+n)', pseu_val):
                                        pseu_val = AnPlusB.parse(pseu_val)
                                    elif re.match(r'(\d+)'):
                                        pseu_val = int(pseu_val)

                                checker_list.append(lambda t, ts, tsf: PSEUDO_TYPE_CHECKER(pseu_val)[pseu_sel]["f"](t, ts, tsf))

                        checker = MultiChecker(checker_list, pseudo_type == "is").do_some_checking
                    else:
                        raise NotImplementedError(
            ...
            for tag in current_context:
                ...
                matches = match_gen(_use_candidate_generator(tag), True)
                errthang = match_gen(_use_candidate_generator(tag), False)
                ...
                for candidate in _use_candidate_generator(tag):
                    ...
                    if checker is not None:
                        try:
                            ...
                            result = checker(candidate, errthang, matches)
